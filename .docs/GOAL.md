...
- 2026-09-08 — CX-01 — Tile-local X, `+`, and Path controls appear while that tile is hovered or selected (or its `+` / Path pull is in progress). Hover does not select. Present and Both still hide them. — approved by user
- 2026-09-08 — NA-03 — A new Before-origin Step created from a parent Step inherits that parent’s Before Who on both lanes. Empty-board Add Step still uses last-used Human, else Alice, else the first Human. A child spawned from Data uses that same fallback. After-only Steps still default to the default Robot. — approved by user
- 2026-09-08 — WG-07 — The pulled `+` taffy has an ink (`--line`) outline along its long edges, matching tile and tab borders. Round caps stay unstroked so the ribbon meets the tile and the Step/Data preview without a wraparound border. — approved by user
- 2026-09-08 — P-08, CX-07 — Board cursors are the platform’s standard ones: `move` on editable tiles (including while dragging), `grab` / `grabbing` on empty paper and `+` / Path tabs, `pointer` on controls such as the tile X. Custom drawn cursor images are not used. — approved by user
- 2026-09-08 — NA-05, NA-10 — A new Other Step (empty-board Add Step, `+` / `1` child, After-only) gets Name “Task” so the tile shows that word. Choosing Type Other on a Step whose Name is empty fills “Task” once; a Name the user already typed is kept. The tile still never prints the word Other. The Other icon is a clipboard (generic work), drawn a little larger than the other Type icons. — approved by user
- 2026-09-08 — WG-07 — The pulled `+` taffy outline is a 3px `--line` band fully outside the green fill, matching tile and tab borders, so it reads on the dim scrim. A centered hairline on the long edges is not enough. — approved by user
- 2026-09-08 — NA-03 — A child Step spawned from Data inherits Who from the nearest upstream Step. If there is no upstream Step, last-used Human, else Alice, else the first Human. — approved by user
- 2026-09-08 — P-08, CX-07 — `move` and `pointer` stay the platform cursors. `grab` / `grabbing` use a white-fill, black-outline hand in the same language as the OS pointer, because Windows has no native grab cursor and Chrome’s bitmap is 1×. — approved by user
- 2026-09-08 — NG-02, CX-05 — While dragging a Step or Data, Paths that touch that Tile are not drop targets and fade. A Path that only shares an ELK trunk or inbound merge stays a drop target on its unique segments after the split (or before the merge). Hovering the shared trunk or merge does not pick a Path. — approved by user
- 2026-09-08 — WG-07 — Step and Data previews on the pulled `+` fan sit closer to the source tile (short arc from the `+` tab; no extra outward offset left over from the old wedge). — approved by user
- 2026-09-08 — NA-05 — The Read Type icon is an open book (blue covers, cream pages, ink spine). Other remains the clipboard. — approved by user
- 2026-09-08 — SH-04 — Soft blip is Tile create only. Path create/connect (Path-pull from the tile tab, and Connect existing) plays a distinct original zip (rising bandpass noise plus a taut-string triangle sweep). Pop, buzz, two-note, and tick are unchanged. Cues stay original Web Audio at a fixed low volume; never copied samples. — approved by user
- 2026-09-08 — P-05, P-07, NA-09 — The right inspector remains the same rail (not a new panel). It may fold to a thin strip with a chevron so the board can use the extra space. Expanded width stays at most 320 px. Present still hides the inspector and the strip. Closed inspector controls are skipped in Tab order. Selecting a Node or Path does not auto-expand. — approved by user
- 2026-09-08 — NA-08, NA-05, NA-11, P-08 — Double-click a Step Name, Details, actor name, or Human role to edit that field on the tile (inspector stays in sync; actor name/role edits the roster). Double-click the task icon for a Type pie (same Types as the inspector picker). Double-click the actor figure for a Who pie (same assign as inspector Who). Double-click Robot Type for LLM / Agent / Script. Enter on a selected Step still does not guess among fields. Double-click does not zoom. Path double-click still toggles stroke. Inspector Who/Type remain. Present and Both stay read-only. — approved by user
- 2026-09-08 — P-05, P-09, NA-06, AQ-01 — A thin status bar spans the bottom of the window and overlaps the right inspector. It shows the current project name (Untitled when unnamed), a persisted right-click-delete toggle (off by default), an Actors control that opens Manage actors, and the application version from package.json. Present and Both hide Actors. — approved by user
...
- 2026-09-08 — WG-02, WG-03, WG-04 — A nonempty board is a weakly connected DAG. Sources are Tiles with no incoming Path (zero or more). Fan-in is allowed. Connections that would create a cycle, a duplicate Path, a self-loop, or a disconnected island are rejected. — approved by user
- 2026-09-08 — WG-06 — Deleting the last base Tile empties the board. Any other Tile may be removed when the restitch stays one connected piece. A source that would split the board into separate workflows is blocked with that explanation. There is no unique-root that cannot be removed. — approved by user
- 2026-09-08 — NG-02 — Free-form layout and forests remain disallowed. A multi-source connected DAG is allowed. Insert-on-Path no longer special-cases a unique source; cycle and disconnect checks suffice. — approved by user
- 2026-09-08 — SH-09 — A version 1 document that is a connected DAG with multiple sources is valid. Forests and cycles are still rejected. `multiple-roots` is no longer a violation. — approved by user
- 2026-09-08 — SH-14, AQ-01, WG-07 — Selected-tile spawn keys are Q Step left, E Step right, A Data left, D Data right. After: Q/E After-only Step; A/D do nothing. Pan keys default unbound (mouse-drag still pans). Arrow/WASD presets are gone. Keybinds: Reset to defaults; Esc cancels capture; Backspace/Delete clears a bind; a new bind that collides clears the other action. Saved maps live in localStorage only. — approved by user
...
- 2026-09-09 — CX-05 — The first derived layout of a lane is computed from the projection alone (document order). After that, the next ELK pass may use the previous displayed positions as row-stability hints so an existing branch stays on its row when a Tile is forked onto either side. New Tiles use their dock hint. Hints are not saved and are not undo entries. — approved by user
- 2026-09-09 — WG-07 — Left `+` / `Q` / `A` always inserts a **parent** of the selected Tile: every incoming Path is retargeted onto the new Tile, then Path `new → this` is added. With no incoming Path, the new Tile is a source (`new → this`). Forks (an extra Path from this Tile) are right `+` / `E` / `D` only. Left Path-pull still connects an existing Tile (`X → this`). Drag-onto-Path still inserts. After left `+` is After-only: it retargets After-only extra incoming Paths the same way and does not rewrite Before Paths. — approved by user
- 2026-09-09 — NA-03 — A predecessor Step from left `+` / `Q` inherits Who from the successor Tile the same way a child inherits from its parent (Step Who, or nearest upstream Step through Data). Empty-board Add Step still uses last-used Human, else Alice, else the first Human. After-only still defaults to the default Robot. — approved by user
- 2026-09-09 — CX-05 — Row-stability hints also apply when left `+` inserts a parent on a branch row. Right `+` still forks. — approved by user
...
- 2026-09-09 — NA-05, NA-10 — A new Other Step no longer gets Name “Task”. Name stays empty until the user types it (empty Name → no tile headline). Choosing Type Other does not fill Name. The Other icon is a blank clipboard (no writing lines). The inspector Type keypad is packed: icon and name with minimal padding, names unclipped, the 3×3 grid centered in the inspector. — approved by user
- 2026-09-09 — NA-05, NA-06 — Who and Manage actors keys are centered in the inspector as one cluster (Humans still wrap among themselves; Robots still start on their own row). — approved by user
- 2026-09-09 — NA-05, NA-10 — The inspector Type Other key does not print the word Other (clipboard only). The accessible name stays Type Other. Icon size and padding match the other Type keys; the name row is kept as empty space so the clipboard does not drop. — approved by user
- 2026-09-09 — NA-01 — A new Human from Manage actors is named "Human" (no number). The name stays editable. Duplicate names are allowed. — approved by user
- 2026-09-09 — NA-07 — Inspector Step Name and Details have no visible captions. Name is a compact box with the Type word on the left (Other has no prefix) and an underline for the title. Details is an empty box. Accessible names remain Name and Details. — approved by user
- 2026-09-09 — NA-07 — The Step inspector has no visible Step, Type, or Who captions. The Type keypad and Who keys remain; accessible names stay Type and Who. The trash control stays at the top right. — approved by user
- 2026-09-09 — CX-07, AQ-03 — After a successful Tile remove, selection moves to the parent Tile (first incoming Path by displayed position). Repeated Delete walks the chain toward the source. A source with remaining successors selects the first remaining successor. Removing the last Tile clears selection. — approved by user
- 2026-09-09 — NA-05, NA-10 — Inspector Type is a 3×3 of alphabetical keys (including Scan) plus a tall Other on the right. Other stays clipboard-only (no printed word). Drag, Approve, and File stay schema-only unless the selected Step already has that Type. The Type keypad, Name/Details well, and Who keys share one cluster width. — approved by user
- 2026-09-09 — NA-07 — Inspector Name and Details sit in a rounded sub-box. There is no underline. The Type word uses a reserved column sized for the longest Type so the Name field length does not change. Other and Details leave that column empty. Accessible names remain Name and Details. — approved by user
- 2026-09-09 — P-05, P-07, P-08, SH-05 — The hamburger sits in the top-left (leftmost control). Present is a top-right icon button, not a hamburger item. Present still hides inspector, NodeToolbars, merge dock, contextual hints, top bar, and status bar; disables editing; Space toggles Before/After; Escape exits and restores view and selection. — approved by user
- 2026-09-09 — P-09, SH-05 — Dark mode is withdrawn from the hamburger. Light is the shipped chrome. Dark tokens and setColorScheme remain so existing tests can still switch theme. There is no Light/Dark menu item. — approved by user
- 2026-09-09 — P-05, NA-06, SH-05 — Actors is not a hamburger item. Manage actors stays in the inspector. — approved by user
- 2026-09-09 — SH-14, WG-08 — Default Remove Node key is Z (not −). Delete still removes. Ctrl+Z remains Undo. Saved keymaps that already bound Remove Node keep that bind. — approved by user
- 2026-09-09 — P-10 — The Present control may use the original Streamline CC BY 4.0 presenter figure (white fill, ink stroke) in front of a light-blue rounded rect, not a Tabler icon. Other chrome stays Mantine + Tabler. — approved by user
- 2026-09-09 — NG-02, CX-05 — Dragging a Step or Data onto a unique Path segment still inserts on that Path. Dropping on a shared merge or split trunk inserts on the whole bundle (merge: every incoming Path onto T, then T→U; split: S→T, then T inherits the outgoing Paths). Restitch the old neighborhood first. If the dragged Tile already sits on a bundled Path, the shared trunk stays a dead zone. No ELK on pointer move. — approved by user
- 2026-09-09 — P-06 — While a Tile is dragged, the contextual hint strip is hidden. There is no Drop keycap and no Esc Cancel chip. Escape still cancels the drag (CX-08). — approved by user
- 2026-09-09 — P-07, BA-05, AQ-01, AQ-02 — Present is a Compare-style stacked Before (top) / After (bottom) board, still full-bleed and read-only. Each pane has a Tabler expand control at its top-right. Expand fills the window with that lane and tucks the other; the control becomes collapse (show split). Space expands Before, then toggles Before/After. Escape returns to split when a pane is expanded, then exits Present. Compare is unchanged and has no expand controls. Present panes share pan/zoom like Compare. — approved by user
- 2026-09-09 — NA-07 — Inspector Name stays left-aligned in its cream box. The Type word is a lifted cream chip on the right, in a reserved column sized for the longest Type so the Name field length does not change. Other and Details leave that column empty. The Name caret is fully visible inside the box. Accessible names remain Name and Details. — approved by user
- 2026-09-09 — NA-07 — Inspector Type stays on the left of Name, in a blue chip. The chip is one width (longest Type word) so switching Type does not move the Name or Details boxes. The Type word is right-aligned inside that chip. Other and Details leave the column empty (no empty chip). Name caret room is unchanged. — approved by user
- 2026-09-09 — NA-01 — Humans and Robots both edit Name, Color, and Role. Robot Type (LLM / Agent / Script) is not a user control. New-board robots are named Robot with Roles LLM, Script, and Agent. Add robot defaults Role Script. Hidden robotKind stays in the document. — approved by user
- 2026-09-09 — NA-04 — The default Robot remains the first Robot (Robot / LLM on a new board). If none exist when After needs one, a Robot named Robot with Role Script (Type Script) is created in the same undo step. — approved by user
- 2026-09-09 — NA-06 — Manage actors opens from the inspector header Actors control (stickman + Actors, about twice the trash width). Deletion is minus delete-mode; unused-only blockers are unchanged. — approved by user
...
- 2026-09-09 — NA-01, NA-06 — Manage actor editor is Name over Role on the left and a tall color card on the right (fill opens the wheel; eyedropper below). — approved by user
- 2026-09-09 — NA-05, NA-06 — Actors is a text-only inspector header button: next to trash on a Step, alone on the empty inspector, hidden on Data and Path. Manage opened from a Step shows Back next to trash; opened from empty keeps Actors as the toggle (no Back). Add human, Add robot, and delete-mode share one row in equal thirds as headshot keys (heads only) plus a centered trash — not nested Who-cards. — approved by user
- 2026-09-09 — NA-05, NA-06 — Manage opened from a Step shows Back only (no Step trash). Actor delete stays the yellow delete-mode key. Who and Manage actor cards stay on a fixed 4-column cluster so a wrapping row does not move existing cards. — approved by user
