...
test.describe("Improvement 11 Path Delete menu", () => {
  test("selected Path hints include Remove Path, stroke samples, and Right-click delete", async ({ page }) => {
    ...
    await expect(helper).toContainText("Edit text");
    await expect(helper.locator("[data-stroke-toggle]")).toBeVisible();
    await expect(helper).not.toContainText("Dotted / Solid");
    await expect(helper).toContainText("Right-click");
    await expect(helper.locator("[data-mouse-right-click]")).toBeVisible();
