...
test.describe("Improvement 28 — Who select is ink, not yellow", () => {
  test("Manage actors selected card is cream with a chunky ink border", async ({ page }) => {
    ...
    await expect(alice).toHaveCSS("background-color", CREAM_LIGHT);
    await expect(alice).not.toHaveCSS("background-color", YELLOW_LIGHT);
    await expect(alice).toHaveCSS("border-top-color", INK_LIGHT);
    await expect(alice).toHaveCSS("border-top-width", "4px");
    ...
    await expect(alice).toHaveCSS("background-color", CREAM_DARK);
    await expect(alice).not.toHaveCSS("background-color", YELLOW_DARK);
    await expect(alice).toHaveCSS("border-top-color", INK_DARK);
    await page.locator("aside").getByLabel("Name").click();
    await capturePage(page, `${EVIDENCE}/manage-actors-dark-1440.png`);
  });

  test("Step inspector Who selected Alice is cream, not yellow", async ({ page }) => {
    await loadOakPark(page);
